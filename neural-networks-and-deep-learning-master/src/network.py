...
class Network(object):
    ...
    def backprop(self, x, y):
        ...
        for n in np.arange(2, self.num_layers):
            z = zs[-n]
            #sp = sigmoid_prime(z)
            sp = activations[-n] * ( 1 -  activations[-n])
            delta = np.dot( self.weights[-n+1].transpose(), delta ) * sp
            nabla_b[-n] = delta
            nabla_w[-n] = np.dot(delta, activations[-n - 1].reshape(activations[-n - 1].shape[0],1).transpose() )
    ...
    def evaluate(self, test_data):
        ...
        for temp in test_data:
            ...
            test_results.append((test_result,y))
        #test_results = [(np.argmax(self.feedforward(temp[:-1])), temp[-1])  for temp in test_data]
